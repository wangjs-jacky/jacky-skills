...
<gsd:workflow>
  ...
  <gsd:phase name="research_interactive" order="5b" condition="选择交互模式">
    ...
    <gsd:step>理解确认：询问用户是否理解</gsd:step>
    <gsd:step condition="不理解">补充解释和示例</gsd:step>
    <gsd:step condition="理解">进入下一个概念</gsd:step>
...
1. **对话知识实时归档**：有价值的内容自动捕获和沉淀
2. **实时思维导图**：维护知识树，帮助用户建立全局视角
...
> ```
>
> 💾 **已生成**: 思维导图已保存到 `notes/mindmap-{主题}.md`
>
> 我们开始逐步讲解，准备好了吗？
> ```
...
- [ ] 交互模式：分步骤渐进式教学
- [ ] 交互模式：每一步讲解后自动判断内容价值
- [ ] 交互模式：有价值的内容实时归档到 notes/{主题分类}/{问题关键词}.md
- [ ] 交互模式：归档后简要告知用户，不中断教学节奏
- [ ] 交互模式：补充解释和答疑内容同样触发归档更新
...
| Phase 5b | 🔄 理解确认 | Decision | 选择理解/不理解/提问 |
