...
这是 `image-effects` 语义效果目录的长期设计入口。目录由最初 8 个效果扩展为 44 个：保留原有 8 个独立效果，把 Happy 中按“狗狗/提拉米苏”素材分组的 42 个案例去重为 35 个可复用语义效果，并加入独立的撕纸编辑影像拼贴。合并后的源仓库提交仍是公开导出与 Pages 发布的唯一事实源。
